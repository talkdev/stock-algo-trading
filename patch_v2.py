#!/usr/bin/env python3
"""
patch_v2.py -- self-contained repair for the JF-OU / NSE 2026 repository
==============================================================================
...
v2 additions (the command-line edge; the numerics are untouched)
----------------------------------------------------------------
    V1. Bare `python main.py` exited 2 with argparse's "the following arguments are
        required: cmd" -- which reads like a crash when you are following the README
        from a PowerShell window. A no-command invocation now prints the command list
        and runs the read-only `status`, exit 0.
    V2. `python main.py scan --live` failed with "unrecognized arguments: --live"
        because --live was declared only on the top-level parser. Both positions work,
        and they no longer overwrite each other.
    V3. `--date`, `--from` and `--to` are validated and normalised at the boundary:
        2026/09/03, 2026-9-3, 20260903 and a quoted value are accepted, a bad value is
        a one-line message instead of a ValueError from inside pandas, and
        `run --date D` -- previously accepted and silently ignored -- honours D.
    V4. `python main.py --version` prints the engine version and the patch level.

This patch is cumulative: patch_v2.py contains every patch_v1 fix unchanged, so one
run on a fresh checkout does the whole repair, and one run on a tree that already ran
patch_v1 changes nothing but the v2 items.
...
Usage
-----
    python patch_v2.py                     # patch the repository holding this file
    python patch_v2.py C:\\work\\stock-algo-trading
    python patch_v2.py --check             # report only, change nothing
    python patch_v2.py --no-verify
"""
...
tzdata>=2023.3         # IANA tz database for Windows (a no-op on Linux/macOS)''',
    ...
    # ================================================================== v2 additions
    # Everything above is inherited from patch_v1 byte-for-byte, with the same
    # markers, so a tree that already ran patch_v1 reports those fixes as "already
    # correct" and has nothing rewritten. The entries below are what v2 adds, and
    # they all sit on the command-line edge of main.py: the module graph, the
    # numerics, the gates and the trade state machine are untouched.

    e.append(Edit(
        "main.patch-hint", "main.py",
        '''        f"{BASE / 'jfou'}. Run `python patch_v1.py` in the repository root to "
        f"repair the layout."''',
        '''        f"{BASE / 'jfou'}. Run `python patch_v1.py` and then `python patch_v2.py` "
        f"in the repository root to repair the layout."''',
        marker="and then `python patch_v2.py`", must=False))

    e.append(Edit(
        "main.import.datetime", "main.py",
        '''import argparse
import json
import os
import sys
...
def _iso_date(value, label: str) -> str | None:
    ...
    raise SystemExit(f"{label}: {value!r} is not a date. Use YYYY-MM-DD, "
        ...
        marker="def _iso_date("))

    e.append(Edit(
        "main.scan-date", "main.py",
    ...
    as_of = _iso_date(args.date, "--date") or str(session_date_for(now_ist()))''',
        marker='''    as_of = _iso_date(args.date, "--date") or str(session_date_for(now_ist()))
    client = _client(db, args)'''))

    e.append(Edit(
        "main.manage-date", "main.py",
    ...
    e.append(Edit(
        ...
        marker='as_of = _iso_date(getattr(args, "date", None), "--date")'))

    e.append(Edit(
        "main.backtest-dates", "main.py",
...
def main(argv: list[str] | None = None) -> int:
    ...
    add("test", help="run the indicator calibration suite")''',
        marker="def _version_string() -> str:"))

    e.append(Edit(
        "main.no-command", "main.py",
    ...
    ensure_dirs()''',
        marker="if not getattr(args, \"cmd\", None):"))
    return e
